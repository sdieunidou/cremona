...
export const componentsDefaultCopy = {
  title: "Components",
} as const;
...
export interface ComponentsProps extends VisualProps {
  title?: string;
  gradient?: boolean;
  fadeOut?: boolean;
  isometric?: boolean;
...
export function Components({
  title = componentsDefaultCopy.title,
  animated = false,
  trigger = "inView",
  gradient = true,
  fadeOut = false,
    ...
    >
      ...
      >
        <div className="relative p-1.5">
          ...
          <div className="relative rounded-xl border bg-card px-4 py-4 shadow-xs">
            <motion.div
            ...
            >
              <motion.div
                className="flex items-center justify-center rounded-lg border border-border/50 bg-muted/35 p-2"
                variants={animated ? cell : undefined}
              >
                <div className="flex h-3 w-full items-center justify-center rounded-md bg-primary px-1.5">
                  <div className="h-0.5 w-2/3 rounded-full bg-primary-foreground/75" />
                </div>
              </motion.div>
              <motion.div
                className="flex flex-col justify-center gap-1 rounded-lg border border-border/50 bg-muted/35 p-2"
                variants={animated ? cell : undefined}
              >
                <div className="h-0.5 w-1/3 rounded-full bg-muted-foreground/20" />
                <div className="flex h-2.5 items-center rounded-md border bg-card px-1">
              ...
              <motion.div
                ...
                variants={animated ? cell : undefined}
              >
                <div className="flex h-3 w-5.5 items-center justify-end rounded-full bg-primary px-0.5">
                  <div className="size-2 rounded-full bg-primary-foreground" />
                </div>
              </motion.div>
              <motion.div
                className="flex items-center gap-1 rounded-lg border border-border/50 bg-muted/35 p-2"
                variants={animated ? cell : undefined}
              >
                <div className="flex size-2.5 items-center justify-center rounded-xs bg-primary">
              ...
              <motion.div
                ...
                variants={animated ? cell : undefined}
              >
                <div className="size-3 rounded-full bg-primary/15 ring-1 ring-primary/20 ring-inset" />
              ...
              <motion.div
                ...
                variants={animated ? cell : undefined}
              >
                <div className="flex h-3 w-full items-center gap-1 rounded-md border bg-card px-1">
                  <div className="h-0.5 flex-1 rounded-full bg-muted-foreground/20" />
                  <div className="size-1 rounded-xs bg-muted-foreground/30" />
                </div>
              </motion.div>
            </motion.div>
          </div>
