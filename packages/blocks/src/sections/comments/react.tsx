...
const comments = [{ lines: 2 }, { lines: 1, reply: true }, { lines: 2 }];
...
const composer = {
  ...
  visible: { opacity: 1, scale: 1, transition: { duration: 0.35, delay: 0.6, ease: "easeOut" } },
} as const;
...
export function Comments({
  title = commentsDefaultCopy.title,
  count = commentsDefaultCopy.count,
  animated = false,
  trigger = "inView",
  gradient = true,
  fadeOut = false,
      ...
      >
        <div className="relative p-1.5">
          ...
          <div className="relative rounded-xl border bg-card px-4 py-3.5 shadow-xs">
            ...
            >
              <motion.p
                className="text-[10px] leading-none font-semibold text-foreground"
            ...
            >
              {comments.map((comment, i) => (
                <motion.div
                  key={i}
                  className={cn("flex gap-1.5", comment.reply && "ml-4")}
                  variants={animated ? row : undefined}
                >
                  <div className="size-4 shrink-0 rounded-full bg-muted-foreground/15 ring-1 ring-border ring-inset" />
                  <div className="flex flex-1 flex-col gap-1 pt-0.5">
                    <div className="flex items-center gap-1">
                      <div className="h-0.75 w-8 rounded-full bg-foreground/20" />
            ...
            <motion.div
              ...
              variants={animated ? composer : undefined}
              {...state}
            >
              <div className="size-4 shrink-0 rounded-full bg-muted-foreground/15 ring-1 ring-border ring-inset" />
              <div className="flex h-4 flex-1 items-center rounded-full border bg-muted/35 px-2">
                <div className="h-0.5 w-12 rounded-full bg-muted-foreground/20" />
              </div>
              <div className="flex size-4 shrink-0 items-center justify-center rounded-full bg-primary">
                <ArrowUp className="size-2.5 text-primary-foreground" strokeWidth={2.5} />
              </div>
            </motion.div>
          </div>
        </div>
