...
export interface BlogProps extends VisualProps {
  gradient?: boolean;
  fadeOut?: boolean;
  isometric?: boolean;
}
          ...
          <div className="relative rounded-xl border bg-card p-2.5 shadow-xs">
            ...
            >
              {[0, 1, 2, 3].map((i) => (
                <motion.div
                  key={i}
                  className="flex flex-col gap-1 rounded-lg border border-border/50 bg-muted/35 p-1.5"
                  variants={animated ? item : undefined}
                >
                  <div className="h-8 rounded-md bg-muted" />
                  <div className="h-0.75 w-4/5 rounded-full bg-foreground/10" />
                  <div className="h-0.5 w-3/5 rounded-full bg-muted-foreground/15" />
                  <div className="h-0.5 w-2/5 rounded-full bg-muted-foreground/10" />
                </motion.div>
              ))}
            </motion.div>
          </div>
