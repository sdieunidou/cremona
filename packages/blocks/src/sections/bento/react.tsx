...
export const bentoDefaultCopy = {
  title: "Everything you need",
} as const;

const bars = ["h-2", "h-3.5", "h-2.5", "h-4", "h-3"];
...
export interface BentoProps extends VisualProps {
  title?: string;
  gradient?: boolean;
  fadeOut?: boolean;
  isometric?: boolean;
    ...
    >
      ...
      >
        <div className="relative p-1.5">
          ...
          <div className="relative rounded-xl border bg-card px-7 py-5 shadow-xs">
            <motion.div
            ...
            <motion.div
              className="mt-3 grid h-28 grid-cols-3 grid-rows-3 gap-1.5"
              variants={animated ? cells : undefined}
              {...state}
            >
              <motion.div
                className="col-start-1 col-end-3 row-start-1 row-end-3 flex flex-col rounded-lg border border-primary/15 bg-primary/5 p-2 will-change-transform"
                variants={animated ? cell : undefined}
              >
                <div className="size-3 rounded-md bg-primary/20 ring-1 ring-primary/25 ring-inset" />
                <div className="mt-1.5 h-0.75 w-3/4 rounded-full bg-foreground/15" />
                <div className="mt-1 h-0.5 w-1/2 rounded-full bg-muted-foreground/15" />
                <div className="mt-auto flex items-end gap-1">
                  {bars.map((h, i) => (
                    <div key={i} className={cn("w-1 rounded-t-xs bg-primary/30", h)} />
                  ))}
                </div>
              </motion.div>
              <motion.div
                className="col-start-3 row-start-1 flex flex-col rounded-lg border border-border/50 bg-muted/35 p-2 will-change-transform"
                variants={animated ? cell : undefined}
              >
                <div className="size-2.25 rounded-xs bg-primary/15 ring-1 ring-primary/20 ring-inset" />
                <div className="mt-auto h-0.5 w-3/4 rounded-full bg-foreground/10" />
              </motion.div>
              <motion.div
                className="col-start-3 row-start-2 row-end-4 flex flex-col rounded-lg border border-border/50 bg-muted/35 p-2 will-change-transform"
                variants={animated ? cell : undefined}
              >
                <div className="size-2.25 rounded-full bg-primary/15 ring-1 ring-primary/20 ring-inset" />
                <div className="mt-1.5 h-0.5 w-full rounded-full bg-foreground/10" />
                <div className="mt-1 h-0.5 w-2/3 rounded-full bg-muted-foreground/15" />
                <div className="mt-auto h-4 rounded-md bg-muted/50" />
              </motion.div>
              <motion.div
                className="col-start-1 col-end-3 row-start-3 flex items-center gap-2 rounded-lg border border-border/50 bg-muted/35 p-2 will-change-transform"
                variants={animated ? cell : undefined}
              >
                <div className="size-2.5 shrink-0 rounded-full bg-primary/15 ring-1 ring-primary/20 ring-inset" />
