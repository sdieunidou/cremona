...
export type SimpleVariant = "landing" | "dashboard";
      ...
      <div className="flex flex-1 flex-col gap-1.5">
        ...
        <div className="flex flex-1 items-end justify-between gap-0.5 rounded-md bg-muted/60 p-1.5">
          {DASHBOARD_BARS.map((height, i) => (
            <div key={i} className="w-1 rounded-sm bg-chart-3" style={{ height: `${height}%` }} />
          ))}
        </div>
...
export interface SimpleProps extends VisualProps {
  url?: string;
      ...
      >
        ...
        <div className="relative flex flex-col">
          ...
          <motion.div
            ...
            {...state}
          >
            {variant === "landing" ? <LandingPreview /> : <DashboardPreview />}
          </motion.div>
        </div>
      </motion.div>
    </div>
